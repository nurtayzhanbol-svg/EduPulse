...
def setup_session():
    ...
    return session, teacher_token, (alice, alice_token), (bob, bob_token)
...
async def test_student_join_room_with_valid_token_enters_room_and_binds_sid(sio_spy):
    ...
    session, _, (alice, alice_token), _ = setup_session()
    await main.join_room("sid-a", {
        "session_id": session.session_id, "role": "student",
        "student_id": alice.student_id, "student_token": alice_token,
    })
    assert rooms == [("sid-a", session.session_id)]
    assert main._student_sockets["sid-a"] == (session.session_id, alice.student_id)
    assert alice.sid == "sid-a"
...
async def test_student_join_room_without_token_is_rejected(sio_spy):
    emitted, rooms = sio_spy
    session, _, (alice, _), _ = setup_session()
    await main.join_room("sid-a", {"session_id": session.session_id, "role": "student", "student_id": alice.student_id})
    assert rooms == []
...
async def test_student_join_room_with_another_students_token_is_rejected(sio_spy):
    emitted, rooms = sio_spy
    session, _, (alice, _), (bob, bob_token) = setup_session()
    await main.join_room("sid-a", {
        "session_id": session.session_id, "role": "student",
        "student_id": alice.student_id, "student_token": bob_token,
    })
    assert rooms == []
    assert errors(emitted) == ["Invalid student token"]
...
async def test_join_room_cannot_invent_a_student(sio_spy):
    ...
    await main.join_room("sid-x", {
        "session_id": session.session_id, "role": "student",
        "student_id": "Mallory", "student_token": alice_token,
    })
    assert rooms == []
...
async def test_teacher_join_room_requires_teacher_token(sio_spy):
    emitted, rooms = sio_spy
    session, teacher_token, (_, alice_token), _ = setup_session()
...
async def test_telemetry_from_bound_socket_is_processed(sio_spy):
    ...
    await main.telemetry("sid-a", keystroke(session.session_id, alice.student_id, alice_token, count=3))
    assert alice.total_keystrokes == 3
    assert errors(emitted) == []
...
async def test_telemetry_without_join_room_is_rejected(sio_spy):
    emitted, _ = sio_spy
    session, _, (alice, alice_token), _ = setup_session()
    await main.telemetry("sid-unbound", keystroke(session.session_id, alice.student_id, alice_token, count=3))
    assert alice.total_keystrokes == 0
    assert errors(emitted) == ["Unauthorized telemetry"]


async def test_telemetry_with_mismatched_student_name_is_rejected(sio_spy):
    emitted, _ = sio_spy
    session, _, (alice, alice_token), (bob, bob_token) = setup_session()
    await main.join_room("sid-b", {
        "session_id": session.session_id, "role": "student",
        "student_id": bob.student_id, "student_token": bob_token,
    })
    # Bob's socket claims to be Alice (with Bob's own token).
    await main.telemetry("sid-b", keystroke(session.session_id, alice.student_id, bob_token, count=5))
    # ...and even with Alice's stolen token, the socket is bound to Bob.
    await main.telemetry("sid-b", keystroke(session.session_id, alice.student_id, alice_token, count=5))
    assert alice.total_keystrokes == 0
    assert bob.total_keystrokes == 0
...
async def test_telemetry_with_wrong_token_on_bound_socket_is_rejected(sio_spy):
    ...
    await main.telemetry("sid-a", keystroke(session.session_id, alice.student_id, bob_token, count=2))
    await main.telemetry("sid-a", keystroke(session.session_id, alice.student_id, count=2))
    assert alice.total_keystrokes == 0
...
async def test_telemetry_for_other_session_is_rejected(sio_spy):
    emitted, _ = sio_spy
    session, _, (alice, alice_token), _ = setup_session()
    other, _, (other_alice, other_alice_token), _ = setup_session()
    await main.join_room("sid-a", {
        "session_id": session.session_id, "role": "student",
        "student_id": alice.student_id, "student_token": alice_token,
    })
    await main.telemetry("sid-a", keystroke(other.session_id, other_alice.student_id, other_alice_token, count=2))
    assert other_alice.total_keystrokes == 0
    assert errors(emitted) == ["Unauthorized telemetry"]
...
async def test_confusion_spike_alert_is_deduped_within_30s_and_refires_after(sio_spy, monkeypatch):
    ...
    for name in ("Bob", "Carol", "Dave"):
        next(s for s in session.students.values() if s.name == name).status = "yellow"
    await main.join_room("sid-a", {
        "session_id": session.session_id, "role": "student",
        "student_id": alice.student_id, "student_token": alice_token,
    })
    ...
    def send():
        return main.telemetry("sid-a", keystroke(session.session_id, alice.student_id, alice_token, count=1))
    ...
    await main.telemetry("sid-a", {
        "session_id": session.session_id, "student_id": alice.student_id, "student_token": alice_token,
